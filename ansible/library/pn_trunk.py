...
def main():
        module = AnsibleModule(
                argument_spec = dict(
                        ...
                        pn_trunkspeed = dict(required=False, type='str', choices=['disable', '10m', '100m', '1g', '2.5g', '10g', '40g']),
                        pn_trunkegress = dict(required=False, type='str'),
			pn_trunkjumbo = dict(required=False, type='bool'),
			pn_trunklacpmode = dict(required=False, type='str',choices=['off', 'passive', 'active']),
			pn_trunklacppriority = dict(required=False, type='str'),
			pn_trunklacptimeout = dict(required=False, type='str', choices=['slow', 'fast']),
			pn_trunkfallback = dict(required=False, type='str', choices=['bundle', 'individual']),
			...
			pn_trunkhost = dict(required=False, type='bool'),
                ...
                required_if = (
                        [ "pn_trunkcommand", "trunk-create", ["pn_trunkname", "pn_trunkports"] ],
	...
	trunkhost = module.params['pn_trunkhost']
        ...
        if trunkname:
                trunk = cli + trunkcommand + ' name ' + trunkname
	...
	if trunkloopback == True:
		trunk += ' loopback '
	if trunkloopback == False:
		trunk += ' no-loopback '
	...
	if trunkrouting == True:
		trunk += ' routing '
	if trunkrouting == False:
		trunk += ' no-routing '
