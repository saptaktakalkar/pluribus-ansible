...
def create_l2_interface(module, switch, ip, vlan_id, vrrp_id,
    ...
    :param vrrp_id: The vrrp_id which need to be assigned.
    ...
    cli += ' ip %s vlan %s format nic no-show-headers ' % (ip2, vlan_id)
    eth_port = run_cli(module, cli).split()
    eth_port.remove(vrouter_name[0])

    cli = clicopy
    cli += ' vrouter-interface-show vlan %s ip %s ' % (vlan_id, ip1)
    cli += ' vrrp-primary %s format switch no-show-headers ' % eth_port[0]
    existing_vrouter = run_cli(module, cli).split()
    existing_vrouter = list(set(existing_vrouter))
